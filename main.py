...
while True:
    ...
    hero_button_coords = pyautogui.locateOnScreen(
        'data/images/buttons/hero-selection-button.png', confidence=0.6)
    tower_button_coords = pyautogui.locateOnScreen(
        'data/images/buttons/battle-button.png', confidence=0.6)
    tower_screen_detect = pyautogui.locateOnScreen(
        'data/images/buttons/tower-screen.png', confidence=0.6)

    # Clicks the "Battle" button in the main menu
    if battle_button_coords is not None and not finding_game:
        print('a')
    ...
    elif hero_button_coords is not None:
        pyautogui.click(hero_button_coords)
        print("Hero selected")
        time.sleep(2)

    # Clicks the "Battle" button in tower selection
    elif tower_screen_detect is not None and finding_game:
    ...
    back_button_coords = pyautogui.locateOnScreen(
        'data/images/buttons/back-button.png', confidence=0.6)
    if back_button_coords is not None:
        pyautogui.click(back_button_coords)
        print("Discarding chest")
        time.sleep(2)

    ok_button_coords = pyautogui.locateOnScreen(
        'data/images/buttons/ok-button.png', confidence=0.4)
    if ok_button_coords is not None:
        pyautogui.click(ok_button_coords)
        game_counter += 1
        print(
            "Game Finished - Played a total of " +
            str(game_counter) +
            " games")
        time.sleep(2)
    ...
    while pyautogui.locateOnScreen(
        'data/images/ingame/surrender-button.png',
        ...
        if is_right_side is None:
            ...
            is_right_side = pyautogui.locateCenterOnScreen(
                'data/images/ingame/locked-bloon.png', confidence=0.5)[0] > 960

            offset = 0 if is_right_side else 800
            tower_slot_x = 1790 if is_right_side else 130
            print("Playing on the " +
                  ("right" if is_right_side else "left") +
                  " side")
            time.sleep(1)
        ...
        blue_bloon_coords = pyautogui.locateOnScreen(
            'data/images/ingame/blue-bloon.png', confidence=0.7)
        purple_bloon_coords = pyautogui.locateOnScreen(
            'data/images/ingame/purple-bloons.png', confidence=0.7)

        if purple_bloon_coords is not None:

            print("Round 11 - Attempting to rush with purples")

            # Activate Bloon boost
            bloon_boost_coords = pyautogui.locateOnScreen(
                'data/images/ingame/bloon-boost.png', confidence=0.7)
